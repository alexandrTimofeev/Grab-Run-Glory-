...
public class ThirdPersonCharacter : MonoBehaviour
{
    ...
    void HandleAirborneMovement(Vector3 moveAir)
    {
        ...
        moveAir.Normalize();
        //Debug.Log(moveAir);
        if (Mathf.Sqrt(m_Rigidbody.linearVelocity.x * m_Rigidbody.linearVelocity.x + m_Rigidbody.linearVelocity.z * m_Rigidbody.linearVelocity.z) < 2.0f)
            m_Rigidbody.AddForce(moveAir * 7.5f);

        m_GroundCheckDistance = m_Rigidbody.linearVelocity.y < 0 ? m_OrigGroundCheckDistance : 0.01f; //上升的时候不判断是否在地面上
    }
